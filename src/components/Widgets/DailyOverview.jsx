...
function DailyOverview({ currData, index }) {
    // Array of image sources
    const imageSources = [group1, group2, group3, group4];
    const dailyValues = ['721,882', '22,370', '10.604', '50.839']
    const dailyRatio = ['-4.66%', '+0.45%', '-1.07%', '+0.66%']
    return (
        <div className='w-[300px] h-[110px] bg-white rounded-2xl relative'>
            <div className='text-black flex justify-between'>
                <div className='text-black font-Cabin text-xl pl-4 pt-2'>Daily {currData}</div>
                <span className='flex flex-col pr-4'>
                    <div className='pt-1 text-lg'>{dailyValues[index]}</div>
